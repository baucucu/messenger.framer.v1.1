var GETdata, StatusBarLayer, activeAvatar, activeTab, activeTabLabel, activeUsers, apikey, avatar, battery, birthdayUsers, callIcon, calls, callsButton, callsLabel, circle, compose, favoriteUsers, filter, flow, footer, games, gamesButton, gamesIcon, gamesLabel, groupsTab, groupsTabLabel, header, home, homeButton, homeIconActive, homeLabel, homeNotification, homeNotificationLabel, i, item, j, k, l, layer, len, len1, len2, len3, max, message, messageAction, messageActionDelete, messageActionMore, messageActionView, messageText, messageTime, messages, messagesTab, messagesTabLabel, myDay, myDayItem, myDayName, myDayTime, myDays, name, people, peopleButton, peopleIcon, peopleLabel, query, ref, ref1, search, searchBar, searchIcon, searchPlaceholder, signal, sort, statusBar, systemBackgroundColorInactive, systemColorActive, systemColorInactive, systemFontColor, systemFontFamily, systemFontSize, systemTime, tabBar, url, user, users;
...
ref = tabBar.subLayers;
for (j = 0, len1 = ref.length; j < len1; j++) {
  layer = ref[j];
  layer.onTap(function() {
    var inactiveTabs, k, len2, tab;
    inactiveTabs = tabBar.subLayers.filter(function(item) {
      return item !== this;
    });
    for (k = 0, len2 = inactiveTabs.length; k < len2; k++) {
      tab = inactiveTabs[k];
      tab.states.switchInstant("inactive");
      tab.subLayers[0].states.switchInstant("inactive");
    }
    this.states.switchInstant("active");
    return this.subLayers[0].states.switchInstant("active");
  });
}
...
for (l = 0, len3 = users.length; l < len3; l++) {
  user = users[l];
  message = new Layer({
    parent: messages,
    width: Screen.width,
  ...
  message.on(Events.Move, function(offset, layer) {
    if (offset.x > 0) {
  ...
  message.on(Events.DragEnd, function(event, layer) {
    if (event.offset.x > -75) {
      return this.animate("inactive");
    } else {
      return this.animate("active");
    }
  });
  message.onTap(function(event) {
    return this.animate("inactive");
  });
...
callsButton.on(Events.Tap, function(event) {
  return flow.showNext(calls);
});

homeButton.on(Events.Tap, function(event) {
  return flow.showNext(home);
});

peopleButton.onTap(function() {
  return flow.showNext(people);
});

gamesButton.onTap(function() {
  return flow.showNext(games);
});
